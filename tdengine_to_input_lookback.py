...
用法: 填好下方 CONFIG 后运行  python tdengine_to_input_lookback.py
时间范围: start_time / end_time 留空时自动取当前时间之前的四个完整自然日
(今天 19 号 -> 15、16、17、18 号 00:00~00:00); 手动填写则覆盖。

输出:
  <输出名>_full.csv       — 完整导出 (查询窗口内全部数据, 不截取)
...
CONFIG = dict(
    ...
    start_time="2026-07-20 00:00:00",      # 可选: 起始时间 "2026-08-16 00:00:00"; 留空 = 今天-3天 00:00
    end_time="2026-08-17 00:00:00",        # 可选: 结束时间 (左闭右开); 留空 = 今天 00:00
...
def fetch_rows():
    """连接 TDengine, 只查询映射内的 tag, 返回 (df, start_dt, end_dt).

    df: DataFrame [ts, data_poi_name, data_poi_value];
    start_dt / end_dt: 实际生效的查询窗口 (供后续截取 / 统计使用)。
    """
    ...
    start_time = CONFIG["start_time"] or start_dt.strftime("%Y-%m-%d %H:%M:%S")
    end_time = CONFIG["end_time"] or end_dt.strftime("%Y-%m-%d %H:%M:%S")
    ...
    print("各 tag 行数:")
    print(df["data_poi_name"].value_counts().to_string())
...
def main():
    ...
    # ── ① 统计各泵连续运行时长 (完整 4 天数据; 中途停机归 0 重新累计) ──
    ts_series = pd.to_datetime(out["timestamp"])
    hours = [continuous_run_hours(ts_series, out[col]) for col in PUMP_RUN_COLS]
    as_of = end_dt.strftime("%Y-%m-%d %H:%M")
    save_run_hours(RUN_HOURS_FILE, hours, as_of)
    print("已写出:", RUN_HOURS_FILE, " (截至", as_of, ")")
    for i, col in enumerate(PUMP_RUN_COLS):
        print(f"  P{i + 1} ({col}) 连续运行 {hours[i]:.2f} h")

    # ── ② 完整导出: 查询窗口内全部数据, 不截取 (新文件 <输出名>_full.csv) ──
    root, ext = os.path.splitext(out_path)
    full_path = root + "_full" + ext
    out.to_csv(full_path, index=False, encoding="utf-8-sig")
    print("已写出完整数据:", full_path, f"({len(out)} 行)")

    # ── ③ input_lookback 输出: 从完整数据中截取后 3 天 (旧文件名保持不变) ──
    #    例: 窗口含 1、2、3、4 号 -> 输出 2、3、4 号 (最新的 3 天)
    cutoff = end_dt - timedelta(days=LOOKBACK_OUT_DAYS)
    full_n = len(out)
    out = out[out["timestamp"] >= cutoff.strftime("%Y-%m-%d %H:%M:%S")].reset_index(drop=True)
    print(f"截取后 {LOOKBACK_OUT_DAYS} 天 (从 {cutoff} 起): 保留 {len(out)} / {full_n} 行")
