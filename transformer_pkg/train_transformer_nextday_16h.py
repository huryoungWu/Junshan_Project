...
# ============================================================================
# train_transformer_nextday_16h.py — 每天 16 点预测第二天全天的单步自回归训练脚本
...
BASE_CONFIG = {
    ...
    # ── 滑动截止 (2026-09 新增) ──
    "slide_cutoff": True,         # 每天从"前一天 23:00"(第八天 0 小时)逐小时滑到
                                  # "当天 15:00", 共 17 个截止位置 (H=24 + H=1..16),
                                  # 目标始终为第九天 D+1; 样本量 ≈ 天数 × 17。
                                  # 部署仍只用 H=16 (15:00) 一个位置; 想回到
                                  # 每天 1 个样本 (H16 单截止点) 设 False。
...
def make_sequences_ar(processor, index, x_array, y_array, lookback_days, predict_days,
    ...
    任务语义 (本文件):
      ...
      - 多截止点 (use_multicutoff=True): 每天 24 个小时都作截止时刻 (H=1..24),
        H=16 的样本即主任务。
      - 滑动截止 (slide_cutoff=True, 2026-09): 每天 17 个截止位置 ——
        前一天 23:00 (H=24, 第八天 0 小时) + 当天 0:00~15:00 (H=1..16),
        目标始终为第九天 D+1 (H=24 时目标 = 截止天 +2 天)。
    ...
    #   滑动模式 (slide_cutoff): 每天从"前一天 23:00"滑到"当天 15:00",
    #     共 17 个位置 (H=24 及 H=1..16), 目标始终为第九天 D+1
    #   多截止点 = 每个小时; 否则每天固定一个 (主任务, 按天步进)
    slide = processor.config.get("slide_cutoff", False)
    if slide:
        e_candidates = np.arange(lookback_steps - 1, total_len - 1)
        hour_mask = (hours == 23) | (hours <= 15)
        e_candidates = e_candidates[hour_mask[e_candidates]]
    elif use_multicutoff:
        e_candidates = np.arange(lookback_steps - 1, total_len - 1)
    else:
    ...
    for e in e_candidates:
        ...
        if hours_used is not None and H not in hours_used:
            continue
        # 目标天起点: 按时间戳定位; 滑动模式截止在 23:00 (H=24, 第八天 0 小时)
        # 时目标 = 后天 (第九天 D+1), 其余位置目标 = 次日 (同样是 D+1 之后的
        # 下一天 —— 滑动只改变截止时刻在第八天内的位置, 目标天不变)
...
def run_experiment(cfg, x_train_all, y_train_all, x_test_all, y_test_all, processor,
    ...
    print(f"   model_type={cfg.get('model_type', 'transformer')}"
          ...
          f"  |  use_multicutoff={use_multicutoff}"
          f"  |  slide_cutoff={cfg.get('slide_cutoff', False)}"
    ...
    for epoch in epoch_pbar:
        ...
        for batch_x, batch_y, batch_xf, batch_H, batch_totals in batch_pbar:
            ...
            with autocast(enabled=use_amp):
                # 滚到本批最大真实步数: 滑动模式 H=24 样本 total=48, 空洞样本
                # total < 48-H → 用真实 totals 而非 48-H (桶内 H 相同可省填充前向)
                batch_steps = int(batch_totals.max().item())
