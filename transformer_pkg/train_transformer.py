...
#   原仓库根目录下的 train_transformer.py 保持原样, 未做修改。
#
...
BASE_CONFIG = {
    # 军山数据: D:\Junshan_Project\data\水厂2025年小时级汇总.csv
    #   (2025 全年整点小时级, 由 data/merge_data.py 合并 4 个 Excel 对齐生成;
    #    出厂水流量/压力/泵频率为整点实测值, 泵状态为分钟级轮询 ffill 推断到整点 0/1;
    #    4/14 17:00~21:00 缺 5 小时保持 NaN 不虚构)
    ...
    "label": "junshan_L1D_P24H_1h_itransformer",         # 结果子目录名

    "test_days": 30,  # 测试集取最后 N 天: 必须 ≥ 回看+预测天数, 否则测试序列数为 0

    "mape_floor_ratio": 0.1,  # MAPE 过滤: 排除 |true| < 该比例 * max|true| 的点
    # 注: 军山小时流量最小 ~481 m³/h, 无近零值, 该过滤实际不生效 (仅保留口径)
